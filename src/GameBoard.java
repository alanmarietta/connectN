...
public class GameBoard {
	...
	public void displayBoard() {
		...
		printColumnHeaders(board[0].length); // Print slot numbers
	    
		// Print top of the board
	    for (int col = 0; col < board[0].length * 1.5 ; col++) {
	        System.out.print("===");
	    }
	    System.out.print("===");
	    System.out.println();
	    ...
	    for (int row = 0; row < board.length; row++) {
	    	...
	    	// Print row dividers
	    	if (row < board.length - 1) { // Don't print if it's the last row
	    		for (int col = 0; col < board.length * 1.5; col++) {
	    			System.out.print("----");
	    		}
	    		System.out.print("---");
	    		System.out.println();
	...
	// Display the top slot numbers based on # of columns
	private void printColumnHeaders(int columns) {
	...
	// Print out each row and each Location Object's symbol
	private void printRow(int row) {
		System.out.print("|"); // Left border
		
		// Print each location symbol
		for (int col = 0; col < board[row].length; col++) {
			char token = board[row][col].getToken();
